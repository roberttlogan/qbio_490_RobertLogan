...
```{r setup}
    knitr::opts_knit$set(root.dir = normalizePath("/home1/ralogan/490_cluster")) 
```
...
```{r}
write.csv(clinic, "/home1/ralogan/490_cluster/analysis_data/brca_clinical_data.csv", row.names=FALSE)
```

To read the csv back into your environment as a dataframe:

```{r}
clinic <- read.csv("/home1/ralogan/490_cluster/analysis_data/brca_clinical_data.csv")
```
...
# 1. How many rows are there? How many columns? A: 1000 rows, 112 columns
# 2. What is the data type of $gender?A: Str
# 3. What is the age in years of the 5th patient (barcode: TCGA-4H-AAAK)? A: 50
# 4. How old was the 1st patient at their last follow up (barcode: TCGA-3C-AAAU)? HINT: you need to look at data from two columns to answer this A: 45

```{r Exercise 3.1}
colnames(clinic) #112 columns
rownames(clinic) #1,000 rows
```
...
```{r Exercise 4.1}
# create plot here
plot(clinic$last_contact_days_to,clinic$lymph_nodes_examined_count, main = "Correlation betwen last days Contact and Lymph Nodes Examined")
```
...
```{r Exercise 4.2}
# create plot here
boxplot(clinical$birth_days_to)
```
...
```{r}
vital_status_na_mask <-!is.na(clinical$vital_status)
```

2) Create a mask to select for male patients
```{r}
male_mask <- clinical$gender == "male"
```

3) Create a mask to select for patients under the age of 35 OR over 60 at diagnosis. 
```{r}
under30_over60_mask <- clinical$age_at_diagnosis <35 | clinical$age_at_diagnosis > 60
```
...
```{r}
cleaned_clinic$age_category <- ifelse(clinical$age_at_diagnosis < 30, "Young",ifelse(clinical$age_at_diagnosis > 50,"Old","Middle"))
```
...
```{r}
cleaned_clinic$survival_time <- ifelse(clinical$death_days_to == "[Not Applicable]",clinical$last_contact_days_to,clinical$death_days_to)
```
...
```{r}
cleaned_clinic$death_event <- ifelse(clinical$vital_status == "Alive",T,F) 
```
...
Middle aged people appear to have the greatest rate of survival over time
# 4. What age group appears to have the worst rate of survival? 
As time goes on, young people appear to have the worst rate of survival
# 5. What is the approx. 15-year survival rate for "Old" breast cancer patients?
It appears to be around 0.02
...
The p value is 0.011 and since it is < 0.05 the data is statistically significant. The data tells us the longer a patient has cancer, the lower their survival rate is. By comparing each of the different stratas we can see that middle aged people have the best survival rates over time.
